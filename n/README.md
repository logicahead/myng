...
```bash
npm install -g webpack webpack-dev-server karma protractor typings typescript rimraf
...
## Installing
* `clone` this repo
* `npm install -g webpack webpack-dev-server karma protractor typings typescript rimraf` to install required global dependencies
* `npm install` to install local dependencies
* `typings install` to install necessary typings
* `npm run server` to start the dev server in another tab

## Running the app
After you have installed all dependencies you can now run the app. Run `npm run server` to start a local server using `webpack-dev-server` which will watch, build (in-memory), and reload for you. The port will be displayed to you as `http://localhost:4200`.
