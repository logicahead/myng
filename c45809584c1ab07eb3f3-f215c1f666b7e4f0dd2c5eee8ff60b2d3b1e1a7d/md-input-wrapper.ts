...
export class MdInputWrapper implements OnChanges {
    @Input("value")
    _value:string;
    ...
    ngOnChanges(changes) {
        let valueChange = changes._value;

        if (valueChange && !valueChange.previousValue) {
            this.value = valueChange.currentValue; //touch to trigger set value emit
        }
    }
}
...
export class MdInputContainer implements AfterContentInit, OnChanges {
    ...
    @ContentChild(MdInputWrapper)
    _input:MdInputWrapper = null;
